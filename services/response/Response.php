...
class Response {
    ...
    public static function customerQuery (CustomerQueryRequest $request): Customer {
        ...
        while ($stmt->fetch()) {
            ...
            $customer = Response::creditCardsQuery($request);
        }
        $mysqli->close();
        return $customer;
    }
     ...
     * @throws \Exception
     */
    public static function createCreditCard (int $customerId, int $cardId, string $cardNumber, string $cvn, DateTime $expirationDate): CreditCard {
        $expiration = $expirationDate->format('Y-m-d');
        $mysqli = DBConnect::connect();
        $query = 'INSERT INTO creditCard (customerId, id, number, cvn, expirationDate) VALUES (?, ?, ?, ?, ?)';
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('iisss', $customerId, $cardId, $cardNumber, $cvn, $expiration);
        $stmt->execute();
        $creditCard = new CreditCard($cardId, $cardNumber, $expirationDate, $cvn);
        $mysqli->close();
        return $creditCard;
    }
    ...
    public static function createCustomer (
        ...
        $mysqli = DBConnect::connect();
        $query = 'INSERT INTO customer (id, firstname, lastname, birthdate, phone, email, password, street, city, state, zipcode) '
            . 'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('issssssssss', $customerId, $firstname, $lastname, $birth, $phone, $email, $password, $street, $city, $state, $zipcode);
        $stmt->execute();
        $postalAddress = new PostalAddress($street, $city, (new State($state)), (new Zipcode($zipcode)));
        $creditCard = Response::createCreditCard((int) $customerId, (int) $cardId, $cardNumber, $cvn, $expirationDate);
        $customer = new Customer((int) $customerId, $firstname, $lastname, $postalAddress, $email, Factory::buildPhone($phone));
