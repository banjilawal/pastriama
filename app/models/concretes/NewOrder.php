...
class NewOrder extends Entity {
    ...
    public function add (Product $product, int $amount): void {
        ...
        if ($cartQuantity <= 0) {
            $this->addHelper(NewInventory::getInstance()->remove($product, $amount));
        } else if ($cartQuantity > 0 && $cartQuantity < $amount) {
            $this->addHelper($this->getUser()->getCart()->remove($product, $cartQuantity));
            $this->addHelper(NewInventory::getInstance()->remove($product, abs($amount- $cartQuantity)));
        } else {
    ...
    public function randomOrder (): NewOrder {
