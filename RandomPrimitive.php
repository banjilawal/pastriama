...
class RandomPrimitive {
    ...
    private const MIN_PASTRY_PRICE = 1.99;
    private const MAX_PASTRY_PRICE = 3.99;
    ...
    private const ADDRESSES = DATASETS. '\addresses.csv'; //'addresses.csv'; //DATASETS . 'addresses.csv';
...
//        print_r($files);
        ...
        return IMAGES . DIRECTORY_SEPARATOR . $files[$index];
    }
...
//        echo 'postal address:' .$address . PHP_EOL . '<br>';
    ...
    public static function amount (float $floor, float $ceiling): float {
        return $floor + mt_rand() / mt_getrandmax() * ($ceiling - $floor);
    }
...
//        $rating = 3;
        return new Review(self::id(), $user, $pastry, rand(1, 5), $content['title'], $content['text']);
    }
